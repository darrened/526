...
* Now has its own map.
  ...
  * As the robot moves around sensing, it should uncover/update the map 
* Factors its water level in to decision-making. If it does not have enough water to tackle flames then it should start 
heading directly to the water station (using A*). However, it should only do this if it knows the location of the water 
station. If it doesn't, then it should roam around hoping to happen across it. The robot should only use it's own map to 
calculate the path
